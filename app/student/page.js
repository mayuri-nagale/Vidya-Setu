...
export default function StudentPage() {
  ...
  async function handleSubmit(event) {
    ...
    const response = await fetch("/api/auth/login", {
      ...
      body: JSON.stringify({ role: "student", id: event.currentTarget.elements["user-id"].value, password: event.currentTarget.elements.password.value }),
    });
            ...
            <div className="w-full">
              ...
              <form className="mt-7 space-y-5" onSubmit={handleSubmit}>
                <label className="block text-sm font-semibold text-[#365b53]" htmlFor="user-id">
                  Student ID
                  <input id="user-id" name="user-id" type="text" placeholder="Enter your student ID" className="mt-2 h-13 w-full rounded-xl border border-[#d7e2dc] bg-[#fbfcfa] px-4 text-sm font-normal text-[#173b35] outline-none transition placeholder:text-[#a5b1ac] focus:border-[#1d5148] focus:ring-4 focus:ring-[#1d5148]/10" required />
                </label>
